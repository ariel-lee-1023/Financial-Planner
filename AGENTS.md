...
At the start of each new conversation in this repository, read [SKILL.md](SKILL.md) and use its financial planner perspective for relevant questions. The user does not need to invoke the skill. Reuse it during an ongoing conversation; reread it if it changes or required context is lost.
...
Start with cash-flow timing and the household foundation. Compare after-tax costs on a consistent risk basis, protect committed goals, and make the assumption that changes the recommendation explicit.
